...
`, 1)[0], R = !g.trim(), f = 0;
        if (this.options.pedantic ? (f = 2, p2 = g.trimStart()) : R ? f = t[1].length + 1 : (f = t[2].search(this.rules.other.nonSpaceChar), f = f > 4 ? 1 : f, p2 = g.slice(f), f += t[1].length), R && this.rules.other.blankLine.test(d) && (c += d + `
...
`, e = e.substring(Z.length + 1), g = A.slice(f);
        ...
        let O = null, Y;
        this.options.gfm && (O = this.rules.other.listIsTask.exec(p2), O && (Y = O[0] !== "[ ] ", p2 = p2.replace(this.rules.other.listReplaceTask, ""))), i.items.push({ type: "list_item", raw: c, task: !!O, checked: Y, loose: false, text: p2, tokens: [] }), i.raw += c;
      }
      let a = i.items.at(-1);
      if (a)
      ...
      for (let l = 0;l < i.items.length; l++)
        if (this.lexer.state.top = false, i.items[l].tokens = this.lexer.blockTokens(i.items[l].text, []), !i.loose) {
          let c = i.items[l].tokens.filter((g) => g.type === "space"), p2 = c.length > 0 && c.some((g) => this.rules.other.anyLine.test(g.raw));
          i.loose = p2;
        }
      if (i.loose)
        for (let l = 0;l < i.items.length; l++)
...
function makeSnippet(mark2, options) {
  ...
  var match;
  var foundLineNo = -1;
  while (match = re3.exec(mark2.buffer)) {
...
class Locale {
  ...
  async init(lang) {
    ...
    if (typeof window === "undefined") {
      ...
      const retour = await postToServer2("locales/get-all", { lang: prefs2.getLang() });
      if (retour.ok) {
