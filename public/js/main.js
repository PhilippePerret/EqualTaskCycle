...
}, shouldSkip = ([TYPE, type2]) => TYPE === PRIMITIVE && (type2 === "function" || type2 === "symbol"), serializer = (strict, json2, $, _) => {
  const as = (out, value) => {
...
function V(u, e) {
  let t2 = u.replace(m.findPipe, (i2, s, o) => {
    let a = false, l = s;
    for (;--l >= 0 && o[l] === "\\"; )
      a = !a;
    return a ? "|" : " |";
...
function me(u, e, t2, n, r) {
  let i2 = e.href, s = e.title || null, o = u[1].replace(r.other.outputLinkReplace, "$1");
  n.state.inLink = true;
  let a = { type: u[0].charAt(0) === "!" ? "image" : "link", raw: t2, href: i2, title: s, text: o, tokens: n.inlineTokens(o) };
...
`).map((i2) => {
    ...
    if (s === null)
      return i2;
    let [o] = s;
    return o.length >= r.length ? i2.slice(r.length) : i2;
  }).join(`
`);
}
var y = class {
...
${p2}` : p2;
        ...
        if (this.lexer.state.top = true, this.lexer.blockTokens(p2, s, true), this.lexer.state.top = g, n.length === 0)
          break;
        let d = s.at(-1);
        if (d?.type === "code")
          break;
...
` + n.join(`
`), O = this.list(f);
          s[s.length - 1] = O, r = r.substring(0, r.length - d.raw.length) + O.raw, i2 = i2.substring(0, i2.length - R.raw.length) + O.raw, n = f.substring(s.at(-1).raw.length).split(`
`);
          continue;
        }
      }
...
`, 1)[0], R = !g.trim(), f = 0;
        if (this.options.pedantic ? (f = 2, p2 = g.trimStart()) : R ? f = t2[1].length + 1 : (f = t2[2].search(this.rules.other.nonSpaceChar), f = f > 4 ? 1 : f, p2 = g.slice(f), f += t2[1].length), R && this.rules.other.blankLine.test(d) && (c += d + `
...
`, e = e.substring(Z.length + 1), g = A.slice(f);
        ...
        i2.loose || (o ? i2.loose = true : this.rules.other.doubleBlankLine.test(c) && (o = true));
        let O = null, Y;
        this.options.gfm && (O = this.rules.other.listIsTask.exec(p2), O && (Y = O[0] !== "[ ] ", p2 = p2.replace(this.rules.other.listReplaceTask, ""))), i2.items.push({ type: "list_item", raw: c, task: !!O, checked: Y, loose: false, text: p2, tokens: [] }), i2.raw += c;
      }
      let a = i2.items.at(-1);
      if (a)
      ...
      for (let l = 0;l < i2.items.length; l++)
        if (this.lexer.state.top = false, i2.items[l].tokens = this.lexer.blockTokens(i2.items[l].text, []), !i2.loose) {
          let c = i2.items[l].tokens.filter((g) => g.type === "space"), p2 = c.length > 0 && c.some((g) => this.rules.other.anyLine.test(g.raw));
          i2.loose = p2;
        }
      if (i2.loose)
        for (let l = 0;l < i2.items.length; l++)
          i2.items[l].loose = true;
      return i2;
    }
  }
  html(e) {
...
` ? t2[1].slice(0, -1) : t2[1];
  ...
  link(e) {
    ...
    if (t2) {
      ...
      return r = r.trim(), this.rules.other.startAngleBracket.test(r) && (this.options.pedantic && !this.rules.other.endAngleBracket.test(n) ? r = r.slice(1) : r = r.slice(1, -1)), me(t2, { href: r && r.replace(this.rules.inline.anyPunctuation, "$1"), title: i2 && i2.replace(this.rules.inline.anyPunctuation, "$1") }, t2[0], this.lexer, this.rules);
    }
  }
  reflink(e, t2) {
    let n;
  ...
  emStrong(e, t2, n = "") {
    ...
    if (!(r[1] || r[2] || "") || !n || this.rules.inline.punctuation.exec(n)) {
      let s = [...r[0]].length - 1, o, a, l = s, c = 0, p2 = r[0][0] === "*" ? this.rules.inline.emStrongRDelimAst : this.rules.inline.emStrongRDelimUnd;
      for (p2.lastIndex = 0, t2 = t2.slice(-1 * e.length + s);(r = p2.exec(t2)) != null; ) {
...
`) ? "" : `
`) + r.raw, s.text += `
` + r.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = s.text) : t2.push(r), n = i2.length !== e.length, e = e.substring(r.raw.length);
        continue;
      }
...
` + r.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = s.text) : t2.push(r);
  ...
  inlineTokens(e, t2 = []) {
    ...
    for (;e; ) {
      i2 || (s = ""), i2 = false;
      let o;
      if (this.options.extensions?.inline?.some((l) => (o = l.call({ lexer: this }, e, t2)) ? (e = e.substring(o.raw.length), t2.push(o), true) : false))
        continue;
      ...
      if (o = this.tokenizer.reflink(e, this.tokens.links)) {
        ...
        let l = t2.at(-1);
        o.type === "text" && l?.type === "text" ? (l.raw += o.raw, l.text += o.text) : t2.push(o);
        continue;
      }
      ...
      if (this.options.extensions?.startInline) {
        ...
        this.options.extensions.startInline.forEach((g) => {
          p2 = g.call({ lexer: this }, c), typeof p2 == "number" && p2 >= 0 && (l = Math.min(l, p2));
        }), l < 1 / 0 && l >= 0 && (a = e.substring(0, l + 1));
      }
      if (o = this.tokenizer.inlineText(a)) {
        e = e.substring(o.raw.length), o.raw.slice(-1) !== "_" && (s = o.raw.slice(-1)), i2 = true;
        let l = t2.at(-1);
        l?.type === "text" ? (l.raw += o.raw, l.text += o.text) : t2.push(o);
        continue;
      }
      if (e) {
...
`;
  ...
  list(e) {
    ...
    let i2 = t2 ? "ol" : "ul", s = t2 && n !== 1 ? ' start="' + n + '"' : "";
    return "<" + i2 + s + `>
` + r + "</" + i2 + `>
`;
  }
  listitem(e) {
    ...
    if (e.task) {
      ...
      e.loose ? e.tokens[0]?.type === "paragraph" ? (e.tokens[0].text = n + " " + e.tokens[0].text, e.tokens[0].tokens && e.tokens[0].tokens.length > 0 && e.tokens[0].tokens[0].type === "text" && (e.tokens[0].tokens[0].text = n + " " + w(e.tokens[0].tokens[0].text), e.tokens[0].tokens[0].escaped = true)) : e.tokens.unshift({ type: "text", raw: n + " ", text: n + " ", escaped: true }) : t2 += n + " ";
    }
  ...
  parse(e, t2 = true) {
    ...
    for (let r = 0;r < e.length; r++) {
      ...
      if (this.options.extensions?.renderers?.[i2.type]) {
        let o = i2, a = this.options.extensions.renderers[o.type].call({ parser: this }, o);
...
var B = class {
    ...
    return e.forEach((n) => {
      ...
      if (r.async = this.defaults.async || r.async || false, n.extensions && (n.extensions.forEach((i2) => {
        ...
        if ("renderer" in i2) {
          ...
          s ? t2.renderers[i2.name] = function(...o) {
            let a = i2.renderer.apply(this, o);
        ...
        for (let s in n.renderer) {
          ...
          if (["options", "parser"].includes(s))
            continue;
          let o = s, a = n.renderer[o], l = i2[o];
          i2[o] = (...c) => {
            let p2 = a.apply(i2, c);
            return p2 === false && (p2 = l.apply(i2, c)), p2 || "";
          };
        }
        r.renderer = i2;
      }
      if (n.tokenizer) {
        ...
        for (let s in n.tokenizer) {
          ...
          if (["options", "rules", "lexer"].includes(s))
            continue;
          let o = s, a = n.tokenizer[o], l = i2[o];
          i2[o] = (...c) => {
            let p2 = a.apply(i2, c);
            return p2 === false && (p2 = l.apply(i2, c)), p2;
          };
      ...
      if (n.hooks) {
        ...
        for (let s in n.hooks) {
          ...
          if (["options", "block"].includes(s))
            continue;
          let o = s, a = n.hooks[o], l = i2[o];
          S.passThroughHooks.has(s) ? i2[o] = (c) => {
            if (this.defaults.async && S.passThroughHooksRespectAsync.has(s))
              return (async () => {
                let g = await a.call(i2, c);
                return l.call(i2, g);
              })();
            let p2 = a.call(i2, c);
            return l.call(i2, p2);
            ...
            if (this.defaults.async)
              return (async () => {
                let g = await a.apply(i2, c);
                return g === false && (g = await l.apply(i2, c)), g;
              })();
            let p2 = a.apply(i2, c);
            return p2 === false && (p2 = l.apply(i2, c)), p2;
          };
      ...
      if (n.walkTokens) {
        ...
        r.walkTokens = function(o) {
          let a = [];
          return a.push(s.call(this, o)), i2 && (a = a.concat(i2.call(this, o))), a;
        };
      }
  ...
  parseMarkdown(e) {
    return (n, r) => {
      let i2 = { ...r }, s = { ...this.defaults, ...i2 }, o = this.onError(!!s.silent, !!s.async);
